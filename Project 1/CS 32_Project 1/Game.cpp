...
Game::Game(int rows, int cols, int nSnakes)
{
	...
	{
		cout << "***** Cannot create Game with play << nSnakes"
...
void Game::play()
{
	...
	while (!m_pit->player()->isDead() && m_pit->snakeCount() > 0)
	{
		...
		else
		{
			...
			case 'q':
				return;
			case 'h':	//if calls history
				m_pit->history().display();	//display the History grid
				cout << "Press enter to continue.";
				cin.ignore(10000, '\n');	//after History grid is shown, press enter '\n' to continue
				clearScreen();	//clear the History gird
				continue;	//back to the Game grid
			case 'u':
			case 'd':
			case 'l':
			case 'r':
