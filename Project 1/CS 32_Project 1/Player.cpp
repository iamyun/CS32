...
void Player::move(int dir)
{
	...
	if (m_pit->numberOfSnakesAt(m_row + rowDelta, m_col + colDelta) == 0)
	{
		m_row += rowDelta;
		m_col += colDelta;
		m_pit->history().record(m_row, m_col);	//record the move after moved to a grid point without jumping
		return;
	}
	...
	if (maxCanMove >= 2)  // need a place to land
	{
		...
		m_row += 2 * rowDelta;
		m_col += 2 * colDelta;
		m_pit->history().record(m_row, m_col);	//record the move after landed at a grid point after jumping over a snake
